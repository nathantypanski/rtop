...
pub fn read_stat(file: &mut File) -> Cpu {
    ...
    let contents: Vec<String> = bytes
        ...
        .collect();
    let cpu = Cpu::new(
                num::from_str_radix(contents[1].as_slice(), 10).unwrap(),  // user
                num::from_str_radix(contents[2].as_slice(), 10).unwrap(),  // nice
                num::from_str_radix(contents[3].as_slice(), 10).unwrap(),  // system
                num::from_str_radix(contents[4].as_slice(), 10).unwrap(),  // idle
                num::from_str_radix(contents[5].as_slice(), 10).unwrap(),  // iowait
                num::from_str_radix(contents[6].as_slice(), 10).unwrap(),  // irq
                num::from_str_radix(contents[7].as_slice(), 10).unwrap()); // softirq
